...
// Un agent ne voit que ses propres biens
if (!is_admin()) {
    $filters['managed_by'] = $_SESSION['user_id'];
}
...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 12px;">
    ...
    <a href="/admin/property-form.php" class="btn btn-gold">
    ...
    <div style="padding: 18px 24px;">
        <form method="GET" action="/admin/properties.php" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: flex-end;">
            <div class="form-group" style="flex: 1; min-width: 180px; margin-bottom: 0;">
                <label style="font-size: 11px; color: var(--gray-500); text-transform: uppercase; letter-spacing: 0.8px;">Recherche</label>
                <input type="text" name="search" value="<?= htmlspecialchars($searchVal, ENT_QUOTES | ENT_HTML5) ?>"
                       placeholder="Titre, lieu..." style="padding: 9px 12px; border: 1px solid var(--gray-300); border-radius: 6px; font-size: 14px; outline: none;"
            ...
            <div class="form-group" style="min-width: 170px; margin-bottom: 0;">
                <label style="font-size: 11px; color: var(--gray-500); text-transform: uppercase; letter-spacing: 0.8px;">Type</label>
                <select name="type" style="padding: 9px 12px; border: 1px solid var(--gray-300); border-radius: 6px; font-size: 14px; outline: none; background: white;">
                    <option value="">Tous les types</option>
                    <option value="maison"       <?= $typeVal === 'maison'       ? 'selected' : '' ?>>Maison</option>
                    <option value="appartement"  <?= $typeVal === 'appartement'  ? 'selected' : '' ?>>Appartement</option>
                    <option value="terrain"      <?= $typeVal === 'terrain'      ? 'selected' : '' ?>>Terrain</option>
                    <option value="villa"        <?= $typeVal === 'villa'        ? 'selected' : '' ?>>Villa</option>
                    <option value="chalet"       <?= $typeVal === 'chalet'       ? 'selected' : '' ?>>Chalet</option>
                </select>
            </div>
            <div class="form-group" style="min-width: 170px; margin-bottom: 0;">
                <label style="font-size: 11px; color: var(--gray-500); text-transform: uppercase; letter-spacing: 0.8px;">Statut</label>
                <select name="status" style="padding: 9px 12px; border: 1px solid var(--gray-300); border-radius: 6px; font-size: 14px; outline: none; background: white;">
            ...
            <a href="/admin/properties.php" class="btn btn-outline" style="padding: 9px 16px; font-size: 13px;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php if ($p['main_image']): ?>
                        <br><span style="font-size: 11px; color: var(--gray-400);"><i class="fas fa-image"></i> Image</span>
                        <?php endif; ?>
                    </td>
                    ...
                    <td style="font-weight: 600; color: var(--gold); white-space: nowrap;">
                        <?= number_format((float)$p['price'], 0, ',', ' ') ?> €
                    </td>
                    <?php if (is_admin()): ?>
                    <td style="font-size: 13px;"><?= htmlspecialchars($p['manager_name'], ENT_QUOTES | ENT_HTML5) ?></td>
                    <?php endif; ?>
                    <td>
                    ...
                    <td>
                        <div class="action-btns">
                            <a href="/admin/property-form.php?edit=<?= (int)$p['id'] ?>" class="btn-icon edit" title="Modifier">
                                <i class="fas fa-pencil-alt"></i>
                            </a>
                            <button class="btn-icon delete"
                                    data-delete-url="/admin/property-delete.php"
                                    data-delete-name="<?= htmlspecialchars($p['title'], ENT_QUOTES | ENT_HTML5) ?>"
                                    data-csrf="<?= htmlspecialchars(csrf_generate(), ENT_QUOTES | ENT_HTML5) ?>"
                                    title="Supprimer"
                                    onclick="
                                        ...
                                        confirmAction(
                                            ...
                                            () => {
                                                ...
                                                f.method='POST'; f.action='/admin/property-delete.php';
                ...
                <tr>
                    <td colspan="<?= is_admin() ? 8 : 7 ?>" style="text-align: center; color: var(--gray-400); padding: 40px;">
                        <i class="fas fa-home" style="font-size: 24px; margin-bottom: 8px; display: block;"></i>
...
<div class="pagination">
    ...
    $baseParams = http_build_query(array_filter([
        'type' => $typeVal, 'status' => $statusVal, 'search' => $searchVal
    ]));
    $sep = $baseParams ? '&' : '';

    if ($page > 1):
    ?>
        <a href="/admin/properties.php?<?= $baseParams ?><?= $sep ?>page=<?= $page - 1 ?>"><i class="fas fa-chevron-left"></i></a>
    <?php endif; ?>

    <?php for ($i = 1; $i <= $result['last_page']; $i++): ?>
        <a href="/admin/properties.php?<?= $baseParams ?><?= $sep ?>page=<?= $i ?>"
           class="<?= $i === $page ? 'active' : '' ?>">
            <?= $i ?>
        </a>
    <?php endfor; ?>
    ...
    <?php if ($page < $result['last_page']): ?>
        <a href="/admin/properties.php?<?= $baseParams ?><?= $sep ?>page=<?= $page + 1 ?>"><i class="fas fa-chevron-right"></i></a>
    <?php endif; ?>
</div>
<?php endif; ?>

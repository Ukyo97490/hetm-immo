...
$recentStmt = $db->prepare(
    "SELECT p.*, CONCAT(u.first_name, ' ', u.last_name) as manager_name
     FROM properties p LEFT JOIN users u ON p.managed_by = u.id
...
<div class="stats-grid">
    <div class="stat-card">
        ...
        <div><div class="stat-value"><?= $stats['total_properties'] ?></div><div class="stat-label">Biens au total</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon gold"><i class="fas fa-check-circle"></i></div>
        <div><div class="stat-value"><?= $stats['active_properties'] ?></div><div class="stat-label">Biens actifs</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon orange"><i class="fas fa-handshake"></i></div>
        <div><div class="stat-value"><?= $stats['sold_properties'] ?></div><div class="stat-label">Biens vendus</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon blue"><i class="fas fa-users"></i></div>
        <div><div class="stat-value"><?= $stats['total_agents'] + $stats['total_admins'] ?></div><div class="stat-label">Utilisateurs actifs</div></div>
    </div>
</div>
...
<div class="table-wrapper" style="margin-bottom:28px;">
    ...
    <div class="table-scroll">
        <table>
            <thead><tr><th>Titre</th><th>Type</th><th>Lieu</th><th>Prix</th><th>Gestionnaire</th><th>Statut</th></tr></thead>
            <tbody>
    ...
    <div class="table-scroll">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td style="font-size:12px;color:var(--gray-500);"><?= date('d/m/Y H:i', strtotime($log['created_at'])) ?></td>
                    <td style="font-size:13px;"><?= htmlspecialchars($log['user_email'] ?? '— système', ENT_QUOTES|ENT_HTML5) ?></td>
                    <td><span class="badge badge-<?= str_contains($log['action'],'fail') ? 'inactive' : 'active' ?>"><?= htmlspecialchars($log['action'], ENT_QUOTES|ENT_HTML5) ?></span></td>
                    <td style="font-size:12px;color:var(--gray-500);"><?= htmlspecialchars($log['target'] ?? '—', ENT_QUOTES|ENT_HTML5) ?></td>
                    <td style="font-size:12px;color:var(--gray-400);"><?= htmlspecialchars($log['ip_addr'], ENT_QUOTES|ENT_HTML5) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>

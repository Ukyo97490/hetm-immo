...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') { http_response_code(405); header('Location: /admin/users.php'); exit; }
csrf_check();

$userId = sanitize_int($_POST['user_id'] ?? null);
$action  = $_POST['action'] ?? '';

if ($userId === null || !in_array($action, ['activate','deactivate'], true)) { $_SESSION['flash_error']='Requête invalide.'; header('Location: /admin/users.php'); exit; }
if ((int)$userId === (int)$_SESSION['user_id']) { $_SESSION['flash_error']='Vous ne pouvez pas désactiver votre propre compte.'; header('Location: /admin/users.php'); exit; }

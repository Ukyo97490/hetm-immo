...
// Count
$total = (int)$db->prepare("SELECT COUNT(*) as c FROM audit_log $where")->execute($params) ? 0 : 0;
    ...
    <div style="padding: 16px 24px;">
        <form method="GET" action="/admin/audit.php" style="display: flex; gap: 12px; align-items: flex-end; flex-wrap: wrap;">
            <div class="form-group" style="min-width: 200px; margin-bottom: 0;">
                <label style="font-size: 11px; color: var(--gray-500); text-transform: uppercase; letter-spacing: 0.8px;">Filtrer par Action</label>
                <select name="action" style="padding: 9px 12px; border: 1px solid var(--gray-300); border-radius: 6px; font-size: 14px; outline: none; background: white;">
                    <option value="">Toutes les actions</option>
                    <?php foreach ($allActions as $a): ?>
                    <option value="<?= htmlspecialchars($a, ENT_QUOTES | ENT_HTML5) ?>"
                            <?= $filterAction === $a ? 'selected' : '' ?>>
                        <?= htmlspecialchars($a, ENT_QUOTES | ENT_HTML5) ?>
                    </option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <?php if ($filterAction): ?>
            <a href="/admin/audit.php" class="btn btn-outline" style="padding: 9px 16px; font-size: 13px;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-size: 13px;">
                        ...
                        <?php else: ?>
                            <span style="color: var(--gray-400);">— Système</span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <?php
                        $badgeClass = 'badge-active';
                        if (str_contains($log['action'], 'fail') || str_contains($log['action'], 'denied'))
                    ...
                    <td style="font-size: 12px; color: var(--gray-500); max-width: 200px;">
                        <?= htmlspecialchars($log['details'] ?? '—', ENT_QUOTES | ENT_HTML5) ?>
                    </td>
...
<div class="pagination">
    $baseQ = $filterAction ? 'action=' . urlencode($filterAction) . '&' : '';

    <?php if ($page > 1): ?>
        <a href="/admin/audit.php?<?= $filterAction ? 'action=' . urlencode($filterAction) . '&' : '' ?>page=<?= $page - 1 ?>"><i class="fas fa-chevron-left"></i></a>
    <?php endif; ?>

    <?php for ($i = 1; $i <= $lastPage; $i++): ?>
        <a href="/admin/audit.php?<?= $filterAction ? 'action=' . urlencode($filterAction) . '&' : '' ?>page=<?= $i ?>"
           class="<?= $i === $page ? 'active' : '' ?>">
            <?= $i ?>
        </a>
    <?php endfor; ?>
    ...
    <?php if ($page < $lastPage): ?>
        <a href="/admin/audit.php?<?= $filterAction ? 'action=' . urlencode($filterAction) . '&' : '' ?>page=<?= $page + 1 ?>"><i class="fas fa-chevron-right"></i></a>
    <?php endif; ?>
</div>
<?php endif; ?>
